...
const sections: PrivacySection[] = [
  ...
  },
  {
    ...
    content: "회사는 개인정보 수집 및 이용 목적이 달성된 후에는 해당 정보를 지체 없이 파기합니다. 단, 다음의 경우에는 명시된 기간 동안 보유합니다.",
    items: [
  ...
  },
  {
    ...
    content: "회사는 이용자에게 맞춤 서비스를 제공하기 위해 쿠키를 사용할 수 있습니다. 이용자는 브라우저 설정을 통해 쿠키를 허용하거나 거부할 수 있습니다.",
  },
  {
    title: "제9조 (개인정보 보호책임자)",
    content: "회사는 개인정보 처리에 관한 업무를 총괄해서 책임지고, 아래와 같이 개인정보 보호책임자를 지정하고 있습니다.",
    items: ["개인정보 보호책임자: HS WEB 대표", "연락처: 문의하기 페이지를 통해 연락"],
  },
  {
    title: "제10조 (방침의 변경)",
    content: "본 개인정보처리방침은 법령, 정책 또는 보안 기술의 변경에 따라 내용이 추가, 삭제 및 수정될 수 있으며, 변경 시 웹사이트를 통해 공지합니다.",
  },
];
        ...
        <div className="px-4 py-5 space-y-5 text-[13px] text-[var(--color-text-2)] leading-relaxed">
          {sections.map((s, i) => (
            <div key={i}>
              ...
              {s.items && (
                <ol className="list-decimal list-outside pl-5 space-y-0.5 mt-1.5">
                  {s.items.map((it, j) => (
                    <li key={j}>{it}</li>
                  ))}
                </ol>
              ...
              {s.subsections && (
                <div className="space-y-2 mt-2">
                  {s.subsections.map((sub, j) => (
                    <div key={j}>
                      <p className="text-[13px] font-semibold text-[var(--color-text)] mt-1 mb-0.5">{sub.subtitle}</p>
