...
export default function AdminDashboard() {
  ...
  const [maintenanceUndo, setMaintenanceUndo] = useState<{ id: string; client_id: string; name: string } | null>(null);
  ...
  // 대시보드에 보이는 프로젝트 상태를 한 번에 옮긴다 — 목록·상태별 개수·상태별 목록이 서로 맞아야 한다.
  const applyProjectStatus = (id: string, from: string, to: string) => {
    setStats((prev) => {
      if (!prev) return prev;
      const projectsByStatus = { ...prev.projectsByStatus };
      const moved = projectsByStatus[from]?.find((p) => p.id === id);
      if (moved) {
        projectsByStatus[from] = projectsByStatus[from].filter((p) => p.id !== id);
        projectsByStatus[to] = [...(projectsByStatus[to] ?? []), { ...moved, status: to }];
      }
      return {
        ...prev,
        activeProjects: prev.activeProjects.map((p) => (p.id === id ? { ...p, status: to } : p)),
        projectStatusCounts: {
          ...prev.projectStatusCounts,
          [from]: Math.max(0, (prev.projectStatusCounts[from] ?? 0) - 1),
          [to]: (prev.projectStatusCounts[to] ?? 0) + 1,
        },
        projectsByStatus,
      };
    });
  ...
  const saveProjectStatus = async (
    project: { id: string; client_id: string; name: string },
    from: string,
    to: string
  ) => {
    setStatusError(null);
    setStatusSavingId(project.id);
    applyProjectStatus(project.id, from, to);
    try {
      const res = await fetch(`/api/clients/${project.client_id}/projects/${project.id}`, {
        method: "PUT",
      ...
      applyProjectStatus(project.id, to, from);
      setStatusError(`"${project.name}" 상태를 바꾸지 못했습니다. 다시 시도해 주세요.`);
      return false;
    } finally {
  ...
  const moveToMaintenance = async (project: { id: string; client_id: string; name: string }) => {
    if (statusSavingId) return;
    const ok = await saveProjectStatus(project, "진행중", "유지보수");
    if (!ok) return;
    if (undoTimer.current) clearTimeout(undoTimer.current);
    setMaintenanceUndo({ id: project.id, client_id: project.client_id, name: project.name });
    undoTimer.current = setTimeout(() => setMaintenanceUndo(null), 5000);
  };

  const undoMaintenance = async () => {
    if (!maintenanceUndo || statusSavingId) return;
    const target = maintenanceUndo;
    if (undoTimer.current) clearTimeout(undoTimer.current);
    setMaintenanceUndo(null);
    await saveProjectStatus(target, "유지보수", "진행중");
  };
        ...
        {(() => {
          ...
          return (
            <div className="bg-white border border-slate-200 overflow-hidden">
              ...
              {maintenanceUndo && (
                <div role="status" className="px-5 py-2.5 bg-violet-50/60 border-b border-violet-100 flex items-center justify-between gap-3">
                  <p className="text-xs text-violet-800 min-w-0 truncate">
                    &quot;{maintenanceUndo.name}&quot; 프로젝트를 유지보수로 바꿨습니다.
                  </p>
                  <button
                    type="button"
                    onClick={undoMaintenance}
                    className="shrink-0 text-xs font-semibold text-violet-800 underline underline-offset-2 bg-transparent border-0 cursor-pointer"
                  >
                    되돌리기
                  </button>
                </div>
              ...
              {statusError && (
                <div role="alert" className="px-5 py-2.5 bg-red-50/60 border-b border-red-100 flex items-center justify-between gap-3">
                  <p className="text-xs text-red-700 min-w-0">{statusError}</p>
                  <button
                    type="button"
                    onClick={() => setStatusError(null)}
                    className="shrink-0 text-xs text-red-700 underline underline-offset-2 bg-transparent border-0 cursor-pointer"
                  >
                    닫기
                  </button>
                </div>
              ...
              {inProgress.length === 0 ? (
                ...
                <ul className="list-none m-0 divide-y divide-slate-100">
                  {inProgress.map((p) => (
                    <li key={p.id} className="flex items-center hover:bg-slate-50 transition-colors group">
                      <Link
                        href={`/admin/clients/${p.client_id}`}
                        className="flex-1 min-w-0 flex items-center justify-between gap-3 pl-5 pr-2 py-3.5 no-underline"
                      >
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-slate-900 truncate">{p.name}</p>
                          <p className="text-xs text-slate-500 truncate">{p.client_name}</p>
                        </div>
                        <span className="shrink-0 inline-flex items-center h-[22px] px-2.5 text-[10px] font-semibold rounded-full bg-slate-900/8 text-slate-700">진행중</span>
                      </Link>
                      <button
                        type="button"
                        onClick={() => moveToMaintenance(p)}
                        disabled={statusSavingId !== null}
                        aria-label={`${p.name} 유지보수로 변경`}
                        className="shrink-0 inline-flex items-center h-[26px] px-2.5 text-[11px] font-semibold rounded-full border border-violet-200 bg-violet-50 text-violet-700 hover:bg-violet-100 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors"
                      >
                        {statusSavingId === p.id ? "변경 중…" : "유지보수로"}
                      </button>
                      <Link
                        href={`/admin/clients/${p.client_id}`}
                        tabIndex={-1}
                        aria-hidden="true"
                        className="shrink-0 pl-2 pr-5 py-3.5 no-underline"
                      >
                        <svg
