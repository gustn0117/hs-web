...
const DEFAULT_PRICE_NOTE = "(호스팅 / 도메인 별도)";
...
export default function QuotationPage() {
  ...
  const [priceNote, setPriceNote] = useState(DEFAULT_PRICE_NOTE);
  ...
  const resetForm = () => {
    ...
    setPriceNote(DEFAULT_PRICE_NOTE);
  };
  ...
  const loadQuotation = (q: SavedQuotation) => {
    ...
    setSpecs(q.specs);
    setNotes(q.notes);
    const savedPrice = q.specs.find((sp) => sp.label === "제작 단가")?.value ?? "";
    const noteMatch = savedPrice.match(/^[\d,]+\s*원\s*(.*)$/);
    setPriceNote(noteMatch ? noteMatch[1] : DEFAULT_PRICE_NOTE);
    setTab("new");
  };
  ...
  const priceSpecValue = priceNote.trim() ? `${fmtNum(subtotal)} 원 ${priceNote.trim()}` : `${fmtNum(subtotal)} 원`;
                  ...
                  <div key={idx}>
                    ...
                    {spec.label === "제작 단가" ? (
                      <div className="flex gap-2">
                        <input
                          value={`${fmtNum(subtotal)} 원`}
                          readOnly
                          aria-label="제작 단가 (견적 합계에서 자동 계산)"
                          className="w-32 shrink-0 px-3 py-2 border border-gray-200 rounded-lg text-sm bg-gray-50 text-[var(--color-dark)]"
                        />
                        <input
                          value={priceNote}
                          onChange={(e) => setPriceNote(e.target.value)}
                          aria-label="제작 단가 뒤 문구"
                          placeholder="비워두면 금액만 표시"
                        ...
                        {priceNote && (
                          <button
                            ...
                            className="shrink-0 px-3 text-xs text-[var(--color-gray)] hover:text-red-500 bg-transparent border border-gray-200 rounded-lg cursor-pointer"
                          >
                            삭제
                          </button>
                        )}
