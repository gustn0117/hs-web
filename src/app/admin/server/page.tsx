      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={refreshing}
              className="flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-200 text-[var(--color-gray)] text-sm rounded-lg cursor-pointer hover:bg-gray-50 transition-all disabled:opacity-50"
            >
              <svg
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                {system.cpu.available && system.cpu.data ? (
                  <p className="text-[var(--color-dark)] text-xl font-bold">
                    {system.cpu.data.load1}
                  </p>
                ) : (
              ...
              <>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(system.cpu.data.usagePercent, 100)}%`,
                    }}
                  />
                </div>
                <p className="text-[var(--color-gray)] text-xs mt-2">
                  {system.cpu.data.cores}코어 · {system.cpu.data.usagePercent}%
                </p>
              </>
            ) : null}
          </div>
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                {system.memory.available && system.memory.data ? (
                  <p className="text-[var(--color-dark)] text-xl font-bold">
                    {system.memory.data.usedGB}/{system.memory.data.totalGB}
                    <span className="text-sm font-normal ml-0.5">GB</span>
                  </p>
                ) : (
                  <p className="text-[var(--color-gray)] text-sm">측정 불가</p>
                )}
              ...
              <>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(system.memory.data.usagePercent, 100)}%`,
                    }}
                  />
                </div>
                <p className="text-[var(--color-gray)] text-xs mt-2">
                  {system.memory.data.usagePercent}% 사용
                </p>
              </>
            ) : null}
          </div>
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                {system.disk.available && system.disk.data ? (
                  <p className="text-[var(--color-dark)] text-xl font-bold">
                    {system.disk.data.usedGB}/{system.disk.data.totalGB}
                    <span className="text-sm font-normal ml-0.5">GB</span>
                  </p>
                ) : (
                  <p className="text-[var(--color-gray)] text-sm">측정 불가</p>
              ...
              <>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(system.disk.data.usagePercent, 100)}%`,
                    }}
                  />
                </div>
                <p className="text-[var(--color-gray)] text-xs mt-2">
                  {system.disk.data.usagePercent}% 사용 ·{" "}
                  {system.disk.data.availableGB}GB 남음
                </p>
              </>
            ) : null}
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                {system.uptime.available && system.uptime.data ? (
                  <p className="text-[var(--color-dark)] text-xl font-bold">
                    {system.uptime.data.days}
                    <span className="text-sm font-normal">일</span>{" "}
                    {system.uptime.data.hours}
            ...
            {system.uptime.available && system.uptime.data ? (
              <p className="text-[var(--color-gray)] text-xs mt-5">
                {system.uptime.data.days}일 {system.uptime.data.hours}시간{" "}
                {system.uptime.data.minutes}분
              </p>
            ) : null}
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[var(--color-dark)] font-semibold flex items-center gap-2">
              <svg
                className="w-5 h-5 text-blue-600"
                fill="none"
                viewBox="0 0 24 24"
          ...
          {!docker.available ? (
            <div className="text-center py-8 text-[var(--color-gray)]">
              <p className="text-sm">Docker 소켓에 연결할 수 없습니다.</p>
              <p className="text-xs mt-1">
                docker-compose.yml에 볼륨 마운트를 확인하세요.
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-2.5 px-3 text-[var(--color-gray)] font-medium">
                      이름
                    </th>
                    <th className="text-left py-2.5 px-3 text-[var(--color-gray)] font-medium">
                      이미지
                    </th>
                    <th className="text-center py-2.5 px-3 text-[var(--color-gray)] font-medium">
                      상태
                    </th>
                    <th className="text-left py-2.5 px-3 text-[var(--color-gray)] font-medium">
                      포트
                    </th>
                    <th className="text-right py-2.5 px-3 text-[var(--color-gray)] font-medium">
                      CPU
                    </th>
                    <th className="text-right py-2.5 px-3 text-[var(--color-gray)] font-medium">
                      메모리
                ...
                <tbody>
                  {docker.containers?.map((c) => {
                    ...
                    return (
                      ...
                      >
                        <td className="py-2.5 px-3 text-[var(--color-dark)] font-medium">
                          {c.name}
                        </td>
                        <td className="py-2.5 px-3 text-[var(--color-gray)] max-w-[200px] truncate">
                          {c.image.split("@")[0]}
                        </td>
                        <td className="py-2.5 px-3 text-center">
                          <span
                            className={`inline-block px-2.5 py-0.5 text-[0.7rem] font-semibold rounded-full ${badge.cls}`}
                          >
                            {badge.label}
                          </span>
                        </td>
                        <td className="py-2.5 px-3 text-[var(--color-dark-2)]">
                          {c.ports.length > 0
                            ? c.ports
                                .map(
                                  (p) => `${p.public}:${p.private}/${p.type}`
                                )
                                .join(", ")
                            : "-"}
                        </td>
                        <td className="py-2.5 px-3 text-right text-[var(--color-dark)]">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            {!system.network.available ? (
              ...
              <div className="space-y-3">
                {system.network.data?.map((iface) => (
                  <div
                    key={iface.name}
                    className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0"
                  >
                    <span className="text-[var(--color-dark)] font-medium font-mono text-sm">
                      {iface.name}
                    </span>
                    <div className="flex items-center gap-4">
                      <span className="flex items-center gap-1.5 text-sm">
                        ...
                        <span className="text-[var(--color-dark-2)]">
                          {iface.rxHuman}
                        </span>
                      </span>
                      <span className="flex items-center gap-1.5 text-sm">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            {!system.ports.available ? (
              ...
              <div className="flex flex-wrap gap-2">
                {system.ports.data?.map((p) => {
                  const label = WELL_KNOWN_PORTS[p.port];
                  return (
                    <span
                      key={p.port}
                      className="px-2.5 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs font-mono text-[var(--color-dark)] hover:bg-gray-100 transition-colors"
                      title={label ? `${p.port} (${label})` : `${p.port}`}
                    >
                      {p.port}
                      {label && (
                        <span className="text-[var(--color-gray)] font-sans ml-1">
                          {label}
                        </span>
                      )}
                    </span>
