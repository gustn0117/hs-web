...
const stats = [
  {
    ...
    num: 150, suffix: "+", label: "완료 프로젝트", subtext: "다양한 산업군에서 성공적 수행", color: "from-blue-400 to-blue-300",
  },
  {
    icon: (
      <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function Stats() {
  ...
  return (
    <section className="py-20 bg-[var(--color-dark)] relative overflow-hidden" ref={ref}>
      ...
      <div className="absolute top-[15%] right-[10%] w-16 h-16 border border-blue-500/10 rounded-lg animate-rotate-slow pointer-events-none" />
      <div className="absolute bottom-[20%] left-[8%] w-10 h-10 border border-blue-500/10 rounded-full animate-float pointer-events-none" style={{ animationDelay: "1s" }} />
      <div className="absolute top-[50%] right-[25%] w-3 h-3 bg-blue-400/20 rounded-full animate-float pointer-events-none" style={{ animationDelay: "2s" }} />
      ...
      <div className="max-w-[1100px] mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {stats.map((s, i) => (
            <div
              key={s.label}
              className={`text-center p-8 rounded-2xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] hover:border-white/[0.12] hover:ring-1 hover:ring-blue-400/10 transition-all duration-500 group ${visible ? 'animate-bounce-in' : 'opacity-0'}`}
