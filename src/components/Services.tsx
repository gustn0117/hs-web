      ...
      <div className="max-w-[900px] mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col gap-3">
          {services.map((s, i) => (
            <div
              key={i}
              className="fade-up flex items-center gap-5 bg-white px-6 py-5 rounded-xl border border-gray-100 transition-all duration-300 hover:border-[var(--color-primary)]/30 hover:shadow-md hover:shadow-emerald-500/5 group"
              style={{ transitionDelay: `${i * 50}ms` }}
            >
              <div className="w-10 h-10 shrink-0 bg-gradient-to-br from-[var(--color-primary)] to-emerald-400 text-white rounded-lg flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300">
                {s.icon}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
                  <h3 className="text-[0.95rem] font-bold text-[var(--color-dark)] whitespace-nowrap">{s.title}</h3>
                  <p className="text-[var(--color-gray)] text-[0.85rem] leading-snug">{s.desc}</p>
                </div>
              </div>
              <div className="hidden md:flex items-center gap-1.5 shrink-0">
                {s.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[0.72rem] px-2.5 py-0.5 bg-[var(--color-light)] text-[var(--color-gray)] rounded-full font-medium group-hover:bg-emerald-50 group-hover:text-[var(--color-primary)] transition-colors duration-300"
                  >
                    {tag}
                  </span>
                ))}
